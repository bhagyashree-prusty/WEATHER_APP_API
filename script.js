...
api_key="07ac0489ea3bcd744e0d193499247b13",
...
aqiList= ['Good', 'Fair', 'Moderate', 'Poor', 'VeryPoor'];
...
function  getWeatherDetails(name, lat, lon, country, state){
    ...
    const aqiText = aqiList[data.list[0].main.aqi -1];
    ...
    `;
        ...
        alert('Failed to fetch Air Quailty Index');
    });
        ...
        currentWeatherCard.innerHTML= `
            <div class="current-weather">
                ...
                <div class="card-footer">
                    <p><i class="fa light fa-calender"></i>${days[date.getDay()]}, ${date.getDate()}, ${months[date.getMonth()]}
                    ${date.getFullYear()}</p>
     ...
     let REVERSE_GEOCODING_URL = `http://api.openweathermap.org/geo/1.0/reverse?lat=${latitude}&lon=${longitude}&limit=1&appid=${api_key}`;
      fetch(REVERSE_GEOCODING_URL).then( res => res.json()).then(data => {
        ...
        if(error.code === error.PERMISSION_DENIED){
          alert('Georaphical permission denied.Please reset location permission to grant access again.');
        }
